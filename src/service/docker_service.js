...
export class DockerService {
  ...
  static start(container_id) {
    return new Promise((resolve, reject) => {
        instance.post("/container/start" + token,{container_id}).then((res) => {
          if(!res.data) reject(new Error(500))
          resolve(res.data);
        });
    })
  ...
  static restart(container_id) {
    return new Promise((resolve, reject) => {
        instance.post("/container/restart" + token,{container_id}).then((res) => {
          if(!res.data) reject(new Error(500))
          resolve(res.data);
        });
    })
  ...
  static remove(container_id) {
    return new Promise((resolve, reject) => {
        instance.post("/container/remove" + token,{container_id}).then((res) => {
          if(!res.data) reject(new Error(500))
          resolve(res.data);
        });
    })
